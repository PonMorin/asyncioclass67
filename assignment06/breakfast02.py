...
async def ToastBread(slices):
    for slice in range(slices):
        ...
        await ApplyButter()
        print(f"{time.ctime()} - Toast", slice + 1, "read")
...
async def main():
    ...
    cook_task = [asyncio.create_task(FryEggs(2)), asyncio.create_task(FryBacon()), asyncio.create_task(ToastBread(2))]
    done, pending = await asyncio.wait(cook_task, return_when= asyncio.ALL_COMPLETED)
    # print(len(done))
